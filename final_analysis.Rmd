...
fit <- glmnet(train_x, train_y, alpha = 0, lambda = lambdas)
plot(fit)
# find the optimal lambda
ridge.fit <- cv.glmnet(train_x, train_y, alpha = 0, lambda = lambdas)
plot(ridge.fit)
...
ridge.pred.train <- predict(ridge.fit, s = optional_lambda, newx = train_x)
ridge.pred.test <- predict(ridge.fit, s = optional_lambda, newx = test_x)

# compute R^2 from true and predicted values
ridge.results <- function(true, predicted, df) {
  ridge.SSE <- sum((predicted - true)^2)
  ridge.SST <- sum((true - mean(true))^2)
  R_square <- 1 - SSE / SST
...
# train data
eval_results(train_y, ridge.pred.train, train_set)
eval_results(test_y, ridge.pred.test, test_set)
