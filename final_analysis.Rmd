...
test_index <- sample(1:nrow(data_rm), as.integer(nrow(data_rm)*0.2))
train_rm <- data_rm[-test_index,]
test_rm <- data_rm[test_index,]

train_rm_x <- data.matrix(train_rm[, names(train_rm) != "SalePrice"])
train_rm_y <- train_rm$SalePrice

test_rm_x <- data.matrix(test_rm[, names(test_rm) != "SalePrice"])
test_rm_y <- test_rm$SalePrice

# PCR
pcr_mod <- pcr(log(SalePrice) ~., data = train_rm, validation = "CV") # try scale = T

summary(pcr_mod)
 
validationplot(pcr_mod, val.type = "MSEP")

pcr_pred <- predict(pcr_mod, data_rm[test_index,], ncomp = 100)

mean(((10^pcr_pred) - test_rm_y)^2)
...
#   filter(s1 > 0) 
#   
# train_red <- train_rm[, colnames(train_rm) %in% row.names(lasso_feat) | names(train_rm) == "SalePrice"]

bag.rf <- randomForest(SalePrice ~ ., data = as.data.frame(train_rm), mtry = 8, importance = T)
...
train_red_x <- train_rm_x[, colnames(train_rm_x) %in% row.names(lasso_feat)]
test_red_x <- test_rm_x[, colnames(test_rm_x) %in% row.names(lasso_feat)]
```
...
p<-length(spline_df)
regressor <- colnames(spline_df[, -c(1)])
form<-as.formula(paste0("SalePrice~",paste0("s(",regressor,")",collapse="+")))

# colnames(factors[, sapply(lapply(factors, unique), length) < 3])

# smoothing splines do not work on categorical variables with levels less than maximum degrees of freedom

factors <- names(Filter(is.factor,spline_df))
nonfactors <- spline_df[ , -which(names(spline_df) %in% factors)]
nonfactors <- colnames(nonfactors[, sapply(lapply(nonfactors, unique), length) > 5])

form<-as.formula(
 paste0("SalePrice~",paste0("s(",nonfactors,",k=5,bs=\"cr\")",collapse="+"),"+",paste0("",factors,collapse="+"),collapse=""))

fit <- mgcv::gam(data=spline_df,formula=form,family=gaussian)
plot(fit)

iter = 100
mse_ns <- rep(0, 1)
adj_r_square_ns <- rep(0, 1)
for (i in 1:iter) {
  spline_fit <- <- gam(SalePrice ~ x + s(z,k=-1,bs="cr"))
  spline_pred <- predict(spline_fit, test_x)
  mse_ns[i] <- mean((spline_pred - test_bos$SalePrice)^2)
  adj_r_square_ns[i] <- summary(spline_fit)$adj.r.squared
}

which.min(mse_ns)
which.max(adj_r_square_ns)
