...
# Do OLS on LASSO
pred <- predict(lasso.fit, test_x, s = 0, type = "response")
...
ridge.pred.test <- predict(ridge.fit, s = optimal_lambda, newx = test_x)
...
ridge.results(test_y, ridge.pred.test, test_set)
...
for (i in 1:iter) {
  spline_fit <- lm(SalePrice ~ bs(~, final_df=i, degree=3), data=final_df)
