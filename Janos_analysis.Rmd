...
numericVarNames <- numericVarNames[!(numericVarNames %in% c("SalePrice", "BldgGrade", "FinBasementGrade", "PresentUse", "MtRainier", "Olympics", "Cascades", "Territorial", "SeattleSkyline", "PugetSound", "LakeWashington", "LakeSammamish", "SmallLakeRiverCreek", "OtherView", "isNew", "isRenovated"))]
...
lassoGrid <- expand.grid(alpha = 1, lambda = test_vals)
...
<!-- ## 7.3 Random forest -->
<!-- ```{r} -->
<!-- # trainIndex <- createDataPartition(train_y, p=.5, list=FALSE, times=1) -->

<!-- # train_x1 <- train_x[trainIndex, ] -->
<!-- # train_y1 <- train_y[trainIndex] -->
<!-- # train_x2 <- train_x[-trainIndex, ] -->
<!-- # train_y2 <- train_y[-trainIndex] -->

<!-- cntrl <- trainControl(method = "repeatedcv", number = 5) -->
<!-- rfGrid <- expand.grid(mtry=10) -->

<!-- # rf_mod <- train(x=train_x1, y=train_y1, method="rf", trControl=cntrl, tuneGrid=rfGrid) -->
<!-- # rf_mod2 <- train(x=train_x2, ) -->
<!-- rf_mod <- randomForest(x=train_x, y=train_y, importance=TRUE) -->
<!-- # rf_mod <- train(x=train_x, y=train_y, method="rf", trControl=cntrl, tuneGrid=rfGrid) -->
<!-- rf_mod -->

<!-- rfPredict <- exp(predict(rf_mod, test_x)) -->

<!-- ``` -->

<!-- ```{r} -->
<!-- rf_mod$results$RMSE -->
<!-- rfRMSE <- sqrt(mean(exp(test_y) - rfPredict)^2) -->
<!-- rfRMSE -->
<!-- ``` -->
